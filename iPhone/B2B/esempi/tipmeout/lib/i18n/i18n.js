...
I18n.missingTranslation = function() {
  ...
  //Tmo.logError(new Error(message));
...
I18n.loadLabels = function(objects, modelName, withColon) {
    var colon = withColon ? ":" : "";
    Ext.each(objects, function(object){
        if(object.name && !object.label){
            object.label = I18n.t('activerecord.attributes.'+modelName.toLowerCase()+'.'+object.name.toLowerCase())+colon;
        }
    });  
};

I18n.tc = function(scope, options){
    var text = I18n.translate(scope, options);
    var regexp = new RegExp("^[\\d]+");
    if (!regexp.exec(text))
        return text;
    var value = regexp.exec(text)[0];
    return text.replace(/^[\d]+/, '<strong class="cl_purple">'+value+'</strong>');
};
